<div class="footer-row1">
<div class="footer-nav">
    <div class="footer-nav__col">
      <div class="footer-nav__col-title">Сервис</div>
      <div class="footer-nav__links">
        <div>
          <a href="" class="footer-nav__link">Личный кабинет</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Регистрация</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Горячая линия</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Договор оферта</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Доставка</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Оплата</a>
        </div>
      </div>
    </div>
    <div class="footer-nav__col">
      <div class="footer-nav__col-title">Информация</div>
      <div class="footer-nav__links">
        <div>
          <a href="" class="footer-nav__link">О магазине</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Прайс-лист</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Поставщикам</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Контакты</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Новости</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Статьи</a>
        </div>
      </div>
    </div>
    <div class="footer-nav__col">
      <div class="footer-nav__col-title">Покупателям</div>
      <div class="footer-nav__links">
        <div>
          <a href="" class="footer-nav__link">Услуги</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Пробники</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Последние поступления</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Товары со скидкой</a>
        </div>
      </div>
    </div>
    <div class="footer-nav__col">
      <div class="footer-nav__col-title">Каталог</div>
      <div class="footer-nav__links">
        <div>
          <a href="" class="footer-nav__link">Жидкий мёд</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Зернистый мёд</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Крем мёд</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Мёд мармелад</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Мёд натуральный в сотах</a>
        </div>
        <div>
          <a href="" class="footer-nav__link">Пчелопродукты</a>
        </div>
      </div>
    </div>
  </div>
</div>

<div class="footer-row2">
  <div class="footer-row2__socials">
    <div class="footer-socials">
      <a href="" class="footer-social" style="background-color: #4D76A1;" rel="nofollow" target="_blank">
        <svg width="21" height="12" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M10.433 11.579h1.178s.355-.04.537-.235c.167-.18.162-.517.162-.517s-.023-1.58.71-1.812c.723-.23 1.65 1.526 2.635 2.201.743.511 1.309.4 1.309.4l2.63-.037s1.376-.085.723-1.167c-.053-.088-.38-.8-1.955-2.263-1.65-1.53-1.429-1.283.558-3.93 1.21-1.613 1.694-2.598 1.543-3.02-.144-.401-1.034-.295-1.034-.295l-2.962.018s-.22-.03-.382.068c-.16.095-.262.318-.262.318s-.469 1.248-1.094 2.309c-1.318 2.24-1.846 2.357-2.061 2.218-.502-.324-.377-1.302-.377-1.996 0-2.17.33-3.075-.64-3.31-.322-.077-.56-.129-1.383-.137-1.056-.01-1.95.003-2.457.251-.337.165-.597.533-.439.554.196.026.64.12.874.44.304.413.293 1.34.293 1.34s.175 2.555-.407 2.872c-.4.218-.947-.227-2.123-2.258C5.407 2.55 4.952 1.4 4.952 1.4s-.087-.215-.244-.33a1.227 1.227 0 00-.455-.184L1.439.904S1.016.916.86 1.1c-.138.163-.01.501-.01.501s2.203 5.155 4.697 7.753c2.288 2.381 4.885 2.225 4.885 2.225z" fill="#fff"></path></svg>
      </a>
      <a href="" class="footer-social" style="background-color: #3B5998;" rel="nofollow" target="_blank">
        <svg width="10" height="21" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.771 10.587H6.094v9.806H2.04v-9.806H.111V7.141h1.928v-2.23C2.04 3.316 2.797.82 6.131.82l3.003.013v3.345h-2.18c-.357 0-.86.178-.86.94v2.027h3.031l-.354 3.443z" fill="#fff"></path></svg>
      </a>
      <a href="" class="footer-social" style="background-color: #55ACEE;" rel="nofollow" target="_blank">
        <svg width="19" height="15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M18.188 1.78a7.154 7.154 0 01-2.06.564A3.595 3.595 0 0017.707.36c-.693.41-1.461.709-2.277.87a3.586 3.586 0 00-6.11 3.27A10.178 10.178 0 011.929.755a3.585 3.585 0 001.11 4.787 3.56 3.56 0 01-1.625-.45v.046A3.587 3.587 0 004.29 8.653a3.573 3.573 0 01-1.619.062 3.59 3.59 0 003.35 2.49A7.194 7.194 0 01.711 12.69a10.141 10.141 0 005.496 1.61c6.595 0 10.201-5.462 10.201-10.2 0-.156-.003-.31-.01-.464a7.271 7.271 0 001.79-1.856z" fill="#F1F2F2"></path></svg>
      </a>
    </div>

    <a href="" class="footer-phone">
      <svg width="24" height="24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M22 16.974v3.01a2.004 2.004 0 01-1.367 1.91c-.266.09-.547.123-.826.098a19.927 19.927 0 01-8.678-3.081 19.59 19.59 0 01-6.034-6.022 19.836 19.836 0 01-3.087-8.701 2.003 2.003 0 011.19-2.017c.256-.112.532-.17.811-.17h3.017a2.014 2.014 0 012.011 1.725c.128.964.364 1.91.704 2.82a2.003 2.003 0 01-.452 2.118L8.012 9.939a16.074 16.074 0 006.033 6.021l1.277-1.274a2.012 2.012 0 012.122-.452c.912.34 1.86.575 2.826.703A2.013 2.013 0 0122 16.974z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>
      <span>+7 (000) 000-00-00</span>
    </a>
  </div>
  <div class="header-row__search">
    <div class="header-row__input">
      <input type="text" name="q" placeholder="Поиск по каталогу" maxlength="50" class="text">
      <a href="">
        <svg class="header-row__btn" width="24" height="24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="11" cy="11" r="8" stroke="#B4B4B4" stroke-width="2" stroke-linejoin="round"></circle><path d="M21.121 21.122l-4.242-4.243" stroke="#B4B4B4" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>
      </a>
    </div>
  </div>
</div>

<div class="footer-row3">
  <div class="footer-copyright">
    © 1999-2022 Logo - мёд оптом
  </div>
</div>