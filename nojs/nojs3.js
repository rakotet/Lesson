...
app.get('/', (req, res) => { // Обрабатываем GET запрос! 1 передаём роут, 2 передаём ф-ю которая будет отвечать на запрос пользователя (в нашем случае просто браузера который делает GET запрос)
    res.render('index') // любая страница с роутом "/" будет рендерится через index.ejs (res.render по умолчанию ищет файлы ejs в папке views)
})

app.post('/', (req, res) => { // обрабатываем POST запрос
    const {city} = req.body // использую деструктаризацию записываем в переменную значение нашей формы отправленное на сервер

    weatherRequest(city) // передаем в нашу импортированную ф-ю значение полученное из формы (город)
    res.render('index') // возвращает отрендеренную страницу
}) 
