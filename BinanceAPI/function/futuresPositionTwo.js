module.exports = async function futuresPositionRiskPampSell(counterPosition, binance, sellMarketCoin, buyMarketCoin, statusOrder, pnlPlusSell, pnlMinusSell, pnlPlusBuy, pnlMinusBuy, timeoutFuturesPositionRisk, profitCounter, currentProfitOne, fs, pnlPlusBuy1, pnlPlusBuy2, pnlPlusBuy3, pnlPlusBuy4) { // авто продажа
  try {
    ...
    for ( let market of markets ) {
      ...
      if ( size != 0 ) {
        let purchaseLevel = 5 // множитель докупки
        ...
        if(positionAmt < 0) {
              ...
              profitCounter[symbol] = 1
                ...
                if(currentProfitOne[symbol] < markPrice) {
                    buyMarketCoin(symbol, positionAmt, binance).then(orderId => {
                      ...
                      console.log(new Date().toLocaleTimeString() + ' Продали Short: ' + symbol + ' По цене: ' + avgPrice + ' - в плюс: ' + counterPosition)
                    })
                  })
                }
              }
          ...
          if(markPrice >= pricePlusBuy) {
            if(dokupkaCounter[symbol] === 0) {
              ...
              dokupkaCounter[symbol] = 0
              buyMarketCoin(symbol, (positionAmt * 3), binance).then(orderId => {
                  statusOrder(symbol, orderId, binance).then(avgPrice => {
                    console.log(new Date().toLocaleTimeString() + ' Открыли Long: ' + symbol + ' По цене: ' + avgPrice)
                  })
                }) 
              } 
            }
              ...
              profitCounter[symbol] = 1
                ...
                if(currentProfitOne[symbol] < markPrice) {
                  sellMarketCoin(symbol, positionAmt, binance).then(orderId => {
                    ...
                    counterPosition++
                    statusOrder(symbol, orderId, binance).then(avgPrice => {
                    console.log(new Date().toLocaleTimeString() + ' Продали Long: ' + symbol + ' По цене: ' + avgPrice + ' - в плюс: ' + counterPosition)
                  })
                })
              }
            }
          ...
          if(markPrice <= priceMinusSell) {
            sellMarketCoin(symbol, positionAmt, binance).then(orderId => {
              ...
              counterPosition--
              statusOrder(symbol, orderId, binance).then(avgPrice => {
                console.log(new Date().toLocaleTimeString() + ' Продали Long: ' + symbol + ' По цене: ' + avgPrice + ' - в минус: ' + counterPosition + '---------------------------------')
              })
            })
          }
        }
...
async function getCandles(coin, binance) { // получить свечи
  try{
    ...
    if(((Number(data[data.length - 2][1]) > Number(data[data.length - 2][4])) && (Number(data[data.length - 1][1]) > Number(data[data.length - 1][4]))) && ((Number(data[data.length - 1][1]) - Number(data[data.length - 1][4])) >= (Number(data[data.length - 1][4]) * 0.001))) {
      return true
    } else {
      return false
    }
